...
if (!class_exists('PaymentCardlinkIrisHelper')) require(JPATH_PLUGINS.DS.'vmpayment'.DS.'cardlinkiris'.DS.'cardlinkiris'.DS.'compatibility'.DS.'vmpsplugin.php');
if (!class_exists('vmText')) require(JPATH_PLUGINS.DS.'vmpayment'.DS.'cardlinkiris'.DS.'cardlinkiris'.DS.'compatibility'.DS.'vmtext.php');
//ini_set('display_errors',1);
//error_reporting(E_ALL);
class plgVmPaymentCardlinkIris extends PaymentCardlinkIrisHelper
{
	...
	function __construct(& $subject, $config) {
		...
		$this->_tablepkey = 'id'; //virtuemart_cardlinkiris_id';
		$this->_tableId = 'id'; //'virtuemart_cardlinkiris_id';
		$varsToPush = array(
			'acquirer' => array(0, 'int'),
			'mid' => array('', 'char'),
			'secretkey' => array('', 'char'),
			'iriscustomercode' => array('', 'char'),
	...
	{
		return $this->createTableSQL('Cardlink Iris Table');
	}
	...
	function plgVmConfirmedOrder($cart, $order)
	{
		...
		$iriscustomercode = $this->_currentMethod->iriscustomercode;
		...
		$post=array(
			...
			'orderDesc'=>$this->get_rf_code( $refID, $this->totalOrder, $iriscustomercode ),
	...
	function get_rf_code( $order_id, $order_total, $iriscustomercode ) {
		$paymentSum = 0;
		if ( $order_total > 0 ) {
		...
		$systemCode   	 = '12';
		$tempCode     	 = $iriscustomercode . $paymentCode . $systemCode . $randomNumber . '271500';		
		$mod97        	 = bcmod( $tempCode, '97' );
		$cd           	 = 98 - (int) $mod97;
		$cd              = str_pad( (string) $cd, 2, '0', STR_PAD_LEFT );
		$rf_payment_code = 'RF' . $cd . $iriscustomercode . $paymentCode . $systemCode . $randomNumber;
